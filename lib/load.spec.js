...
test('PyON with a string containing spaces', t => {
  const input = 'PyON 1 null\n"xyz abc"\n---'
  t.is(load(input).payload, 'xyz abc')
})

test('PyON with a string containing newlines', t => {
  const input = 'PyON 1 null\n"xyz\nabc"\n---'
  t.is(load(input).payload, 'xyz\nabc')
})
...
test('improper PyON input with safeLoad', t => {
  const expected = { type: 'none', payload: 'xyz' }
  t.deepEqual(safeLoad('xyz'), expected)
})

test('whitespace with safeLoad', t => {
  t.deepEqual(safeLoad('\n'), { type: 'empty', payload: null })
})

test('PyON with surrounding whitespace in safeLoad', t => {
  const input = '\nPyON 1 Test\n42\n---\n'
  t.deepEqual(safeLoad(input), { type: 'Test', payload: 42 })
})

test('PyON name containing dash', t => {
  const input = 'PyON 1 simulation-info\n{}\n---'
  t.is(load(input).type, 'simulation-info')
})
